...
signed and null while their signature is outstanding. It is the only per-signer
progress the API reports: `document.status()` says whether everyone is done, not
who.
...
```java
var policy = client.getSignaturePolicy(signer.country());
for (var allowed : policy.signatureTypes()) {
    System.out.println(allowed.type() + " " + allowed.verificationMethods());
}
```
...
`409` (`AutosignlyException.Validation`) rather than duplicating the write.
